# NexSoft Basic Tasks

This repository contains all tasks completed during the NexSoft internship.

## Task 1 - Personal Portfolio Website

### Technologies Used

* Next.js
* TypeScript
* Tailwind CSS
* React
* Responsive Design

### Features

* Modern Portfolio Design
* Animated Hero Section
* About Section
* Skills Section
* Projects Section
* Resume View & Download
* Contact Form UI
* Responsive Layout
* GitHub & Vercel Links

### Project Folder

```bash
personal-portfolio-website
```

---

## Task 2 - Responsive Landing Page

### Technologies Used

* Next.js
* TypeScript
* Tailwind CSS
* React

### Features

* Modern AI Company Landing Page
* Responsive Design
* Dark & Light Mode
* Interactive Service Cards
* Animated Background Effects
* Smooth Animations
* Premium UI/UX Design
* Mobile-Friendly Layout

### Project Folder

```bash
responsive-landing-page
```

---

## Task 3 - Premium To-Do Application

### Technologies Used

* HTML5
* CSS3 (Custom Properties, Flexbox, Grid)
* JavaScript (ES6+, LocalStorage, DOM API)
* Responsive Design

### Features

* SaaS-Grade UI/UX Design
* Light & Dark Mode Support
* Task Priority System (Low, Medium, High)
* Dynamic Filtering (All, Active, Completed)
* Local Storage Persistence
* Fluid Responsive Design (Mobile, Tablet, Desktop)
* Premium Animations & Micro-interactions
* XSS Protected Rendering

### Project Folder

```bash
premium-todo
```

---

## Task 4 - Magic Calc (Modern Calculator)

### Technologies Used

* HTML5
* CSS3 (Custom Properties, Grid, Flexbox)
* JavaScript (ES6+, DOM API, LocalStorage)
* Responsive Design

### Features

* **SaaS-Grade UI:** Modern, polished design with "Space Grotesk" & "Inter" fonts.
* **8+ Premium Color Themes:** Toggle between various light and dark themes (Pink, Blue, Purple, Cyan, Black, etc.).
* **Theme Persistence:** Remembers your color preference using LocalStorage.
* **Full Keyboard Support:** Use your physical keyboard for all operations.
* **Smart Display:** Shows current and previous operations simultaneously.
* **Division by Zero Protection:** Elegant error handling for invalid operations.
* **Fully Responsive:** Optimized for mobile, tablet, and desktop screens.

### Project Folder

```bash
calculator-app
```

---

## Task 5 - SkyCast (Professional Weather App)

### Technologies Used

* HTML5
* CSS3 (Custom Properties, Glassmorphism, Animations)
* JavaScript (ES6+, Fetch API, Async/Await)
* OpenWeatherMap API
* Responsive Design

### Features

* **Real-time Weather Data:** Get current temperature, conditions, and detailed metrics.
* **5-Day Forecast:** Daily weather outlook with noon-time projections.
* **Detailed Weather Insights:** Humidity, Wind Speed, Pressure, Feels Like, High/Low, Sunrise, and Sunset.
* **Dynamic Backgrounds:** Background gradients and overlays change based on the current weather condition (Clear, Clouds, Rain, etc.).
* **Search Functionality:** Search weather for any city globally.
* **Responsive Design:** Fully optimized for mobile, tablet, and desktop viewing.
* **Interactive UI:** Smooth animations, floating blur effects, and glassmorphism design.

### Project Folder

```bash
weather-app
```

---

More tasks will be added in separate folders as they are completed.
